...
민수는 자신감이 넘치는 상태로 `에스프레소`와 `아메리카노`를 구현하기 시작했습니다.
...
설계 단계에서 이 부분은 나중에 바뀔 수도 있을 것 같다(가격 등), 확장이 손쉬우면 좋을 것 같은 부분을 따로 빼서 원칙을 적용하면 됩니다.
...
위 예에서 문제가 되는 부분이 뭐였나요? 새로운 음료를 추가하는 과정은 각각이 Beverage를 상속받는 클래스로 만드는 것이 당연합니다. 하지만 여러가지 옵션이 추가된 음료의 경우 각각에 해당하는 클래스를 만드는게 맞는 걸까요?

민수네 카페에서 상황에 맞게 바뀔 수 있는 부분은 바로 첨가물의 종류입니다. 다르게 말하면 자주 수정되거나 확장될 수 있는 부분이기도 하죠. 이런걸 보면 일단 첨가물을 따로 분리해서 OCP를 지킬 수 있다는 생각이 들죠?

즉, 첨가물 각각을 데코레이터로 만들면 됩니다. 
...
그렇다면 `데코레이터`는 어떤 방식으로 작동하는 걸까요? 위 `데코레이터 패턴`의 정의를 보면 객체에 추가적인 요건을 동적으로 첨가한다고 되어 있습니다. 기존에 동작하는 조건에 어떤 부분을 **추가**해주는 것이죠.

우리 카페에서 첨가물에 따라 추가되는, 다시 말해 바뀔 수 있는 부분이 무엇일까요? 바로 가격이랑 음료에 대한 설명입니다.

민수네 카페에서는 `cost()`와 `getDescription()`가 해당되겠죠?

위에서 아메리카노는 4000원이었고 얼음을 추가하면 500원이 추가되어 총 4500원이라는 가격을 가져야 합니다. `Ice`라는 `데코레이터`가 `Americano`라는 음료를 **꾸며준다고** 생각해봅시다.

`데코레이터`는 어떠한 음료가 들어올지 모르고 지금까지 계산된 해당 음료의 가격에 자신의 가격을 더하는 방법으로 구현되어야 합니다. 즉, `Ice`의 경우 현재 가격에 500원을 추가해서 `return` 해야겠죠?

따라서 `데코레이터`는 `Beverage`를 인스턴스 변수로 가지고 `Beverage`의 `cost()`를 호출한 후 자신의 가격을 더한 값을 리턴하게 됩니다.
...
만약 `데코레이터` 패턴을 사용하지 않고 새로운 음료를 추가했다면 그 음료에 해당하는 `Whip`, `Ice`, `Mocha` 등등 여러 클래스 또한 같이 만들어줬어야 할 겁니다. 또한 얼음의 가격이 올라간다면 그에 해당하는 모든 클래스를 수정해줘야겠죠?

`데코레이터` 패턴을 사용함으로써 클래스의 기능이 동적으로 확장되고 수정될 수 있는 상황에서 재사용성을 높일 수 있는 겁니다!

우리가 구현한 부분을 살펴보면 아래와 같은 구조를 가집니다!

![](../assets/images/../../../assets/images/Post/2020-07-14-16-54-40.png)
